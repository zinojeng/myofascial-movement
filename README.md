...
這個 repo 裝的是一門筋膜課，**主軸是 Thomas Myers 的十二條肌筋膜經線**：
先建立筋膜系統與經線讀法的基礎，中段七章逐條走過淺背線、淺前線、側線、螺旋線、
手臂線、功能線與深前線，最後收在自我放鬆、活動度負荷與課表整合。

每一條線都照同一個順序講：**這條線是什麼 → 它的解剖驗證到哪裡 → 這一區值得練的是什麼 → 實際跟練**。
驗證數字寫在每章開頭，不藏在展開區塊裡——淺前線那一章第一句就是
「7 篇研究、0 個轉換點通過驗證」，然後照樣教前側鏈訓練。
...
| | |
|---|---|
| 章節 | 13（基礎 3 + **十二條經線 7** + 方法與整合 3） |
| 單元 | 40（30 個實作主題、4 個觀念、4 個經線讀法、1 個說明） |
| 影片 | 179 支示範 + 40 堂主課 + 32 堂對照版＝251 個欄位、246 支不重複、137 個頻道 |
| 課程時長 | **20 小時 47 分**（所有語言版本全看過則為 27 小時 31 分） |
| 內容角色 | 251/251 已標註 · 科學與理論 18% · 教練／治療師實務 56% · 純示範 18% · 爭議觀點 3% |
| 來源分級 | 研究機構 12 · 臨床專業 45 · 專業教練 25 · 媒體 2 · 器材品牌 1 · 背景不明 6（使用中頻道 100% 已分級） |
| 字幕 | 246 支中 47 支有人工字幕、224 支至少有自動字幕、**22 支完全沒有**（9%） |
| 實證查核 | 35 個單元層級 + 11 個介入類別＝46 條判定（章節單元 34/34 全數覆蓋） |
| 文獻 | 234 筆結構化引用（153 個不重複 PMID），另有數十個 PMID 直接寫在論述裡 |
| 科學證據 | 證據較充分 **0** · 中等 18 · 有限 14 · 專家共識 1 · 經驗性主張 1 · 存在爭議 3 |
| 驗證 | 稽核零錯誤、連結 246/246 有效、引用 234/234 標題相符、散文 PMID 全數存在 |
